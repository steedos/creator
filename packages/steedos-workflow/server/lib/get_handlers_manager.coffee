...
getHandlersManager.getHandlersByUserAndRole = (user_id, role_id, space_id)->
	orgs = db.organizations.find({space: space_id, users: user_id}).fetch()
	user_ids = new Array
	_.each(orgs, (org)->
...
getHandlersManager.getHandlersByOrgAndRole = (org_id,role_id,space_id)->
	...
	user_ids = new Array
	positions = db.flow_positions.find(space: space_id, org: org_id, role: role_id).fetch()
	...
	if user_ids.length is 0
		...
		_.each(parents, (parent_id)->
			positions = db.flow_positions.find(space: space_id, org: parent_id, role: role_id).fetch()
...
getHandlersManager.getHandlers = (instance_id, step_id) ->
	...
	if deal_type is "applicantRole"
		...
		_.each(org_ids, (org_id)->
			org = db.organizations.findOne(org_id)
			org_children = db.organizations.find({space: space_id, parents: org_id}).fetch()
			...
			_.each(check_orgs, (check_org_user)->
				if check_org_user.users
					_.each(check_org_user.users, (org_user)->
						check_user_count = db.space_users.find({space: space_id, user: org_user}).count()
						if check_org_user is 0
		...
		_.each(valid_approver_org_ids, (valid_approver_org_id)->
			valid_approver_org = db.organizations.findOne(valid_approver_org_id)
			if valid_approver_org.users
				org_user_ids = org_user_ids.concat(valid_approver_org.users)

			child_orgs = db.organizations.find({space: space_id, parents: valid_approver_org_id}).fetch()
		...
		_space_user = db.space_users.findOne({space: space_id, user: instance.applicant})
