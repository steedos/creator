...
(function (root, factory) {
   ...
   * @class WideArea
   */
  function WideArea(obj, areaId) {
    this._targetElement = obj;

    //starts with 1
    this._wideAreaId = areaId || 1;

    if(areaId){
		this._wideAreaIdIsNumber = false
    }else{
		this._wideAreaIdIsNumber = true
    }
  ...
  function _enable() {
    ...
    for (var i = textAreaList.length - 1; i >= 0; i--) {
      ...
      $(currentTextArea).parent().append(wideAreaWrapper);
  ...
  function _enableFullScreen(link) {
    ...
    var wideAreaId = link.parentNode.id.replace(/widearea\-/, "");
  ...
  var wideArea = function (selector, scopeElement, id) {
    if(scopeElement){
		return new WideArea(scopeElement, id);
