...
Meteor.methods
	...
	"reset_grid_width": (object_name, list_view_id)->
		userId = this.userId
		setting = Creator.getCollection("settings").findOne({object_name: object_name, record_id: "object_gridviews", owner: userId})
		column_width = setting.settings[list_view_id].column_width
		keys = _.keys column_width
		_.each keys, (k)->
			column_width[k] = 0
		
		Creator.getCollection("settings").update({_id: setting._id}, {$set: {"settings.#{list_view_id}.column_width": column_width}})